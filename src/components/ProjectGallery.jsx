  ...
  return (
    <>
      ...
      >
        ...
        {/* NDA Creative Link Overlay — always show if NDA and link exists */}
        {project.nda && project.link && (
          <div className="absolute inset-0 flex items-center justify-center bg-black/40 backdrop-blur-[1.5px] transition-all group-hover:bg-black/15 z-10">
            <a
              href={project.link.href}
              target="_blank"
              rel="noopener noreferrer"
              onClick={(e) => e.stopPropagation()}
              className="px-6 py-2.5 bg-[#0d0d0d] border border-[#c07a3f]/50 text-[#c07a3f] font-mono text-[11px] uppercase tracking-widest rounded-sm hover:bg-[#c07a3f] hover:text-[#0d0d0d] transition-all duration-300 flex items-center gap-3 active:scale-95 shadow-2xl shadow-black"
            >
              <span className="w-1.5 h-1.5 rounded-full bg-[#c07a3f] animate-pulse" />
              {project.link.label || 'View Resource'}
              <svg width="12" height="12" viewBox="0 0 12 12" fill="none" stroke="currentColor" strokeWidth="1.5" strokeLinecap="round" strokeLinejoin="round">
